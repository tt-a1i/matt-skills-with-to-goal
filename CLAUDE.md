...
For this fork's maintainer installation, use `npm run sync:local` as described in [docs/maintaining-fork.md](./docs/maintaining-fork.md). `scripts/link-skills.sh` is an inherited upstream development helper for a separate installation layout; running it over the unified skill directory replaces managed copies with repository links. Keep the two layouts separate.
