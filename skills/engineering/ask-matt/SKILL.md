...
3. **Branch — how should implementation cross the context boundary?**
   ...
   - **Multi-session, parallel, cross-agent, delayed, or context already noisy** → **`/to-spec`**, then normally **`/to-tickets`** to split it into tracer-bullet tickets, each declaring its **blocking edges**. On a local tracker that's one file per ticket under `.scratch/<feature>/issues/`, worked blockers-first by hand; on a real tracker the edges become native blocking links, so any ticket whose blockers are done can be grabbed. Run **`/to-goal`** on the current frontier ticket and execute that goal in a clean session, regenerating from the next frontier after each slice. When the user wants one persistent Codex task to complete the whole dependency graph, invoke **`/execute-spec-in-fork`** after the parent and tickets are approved. If the approved large spec already contains dependency-ordered frontiers with independently decidable criteria, invoking it launches that spec persistently without manufacturing redundant tickets. Use `/to-goal --all` first only when the Goal must also be portable outside the inherited task.

   Both **`/implement`** and **`/spec-executor`** drive **`/tdd`** internally — one red-green slice at a time — then close out with **`/code-review`**, a two-axis review (Standards + Spec) of the diff, before committing. `/spec-executor` preserves the bounded Spec contract and receipt; `/execute-spec-in-fork` owns the Codex task lifecycle and applies the same validation and review bar across every frontier in persistent Goal mode. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point.
...
- **`/execute-spec-in-fork`** — the Codex App adapter. It selects a bounded Spec run or an explicitly requested persistent Goal run, creates and names the exact fork, establishes the Messenger return path, resumes decisions, validates the matching final receipt, and archives only a correlated clean completion.
